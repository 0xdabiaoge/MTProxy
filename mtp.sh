...
install_mtp_python() {
    ...
    # 优先检测本地是否有预编译文件 (方便离线/自定义部署)
    LOCAL_BIN_NAME="${TARGET_BIN}"
    if [ -f "./${LOCAL_BIN_NAME}" ]; then
        echo -e "${GREEN}检测到本地二进制文件: ./${LOCAL_BIN_NAME}，跳过下载...${PLAIN}"
        cp "./${LOCAL_BIN_NAME}" "$BIN_DIR/mtp-python"
    elif [ -f "./mtp-python" ]; then
         # 兼容用户直接命名为 mtp-python 的情况
        echo -e "${GREEN}检测到本地二进制文件: ./mtp-python，跳过下载...${PLAIN}"
        cp "./mtp-python" "$BIN_DIR/mtp-python"
    else
        echo -e "${BLUE}正在从 GitHub 下载 Python 版二进制文件...${PLAIN}"
...
install_mtg() {
    ...
    # 优先检测本地是否有预编译文件 (方便离线/自定义部署)
    if [ -f "./mtg-go" ]; then
        echo -e "${GREEN}检测到本地二进制文件: ./mtg-go，跳过下载...${PLAIN}"
        cp "./mtg-go" "$BIN_DIR/mtg-go"
    elif [ -f "./mtg" ]; then
        echo -e "${GREEN}检测到本地二进制文件: ./mtg，跳过下载...${PLAIN}"
        cp "./mtg" "$BIN_DIR/mtg-go"
    else
        # 从用户 GitHub 下载 mtg-go 二进制文件
        echo -e "${BLUE}正在从 GitHub 下载 mtg-go ($MTG_ARCH)...${PLAIN}"
        
        # 新版二进制：已包含所有修复 (only-ipv4, 24h time tolerance, 10s circuit breaker)
        DOWNLOAD_URL="https://github.com/0xdabiaoge/MTProxy/releases/download/mtg-go.mtp-python/mtg-go-${MTG_ARCH}"
        ...
        echo -e "${GREEN}下载并安装成功: mtg-go${PLAIN}"
    fi
...
EOF
    ...
    # 检查 mtg-go 是否能运行
    "$BIN_DIR/mtg-go" --version >/dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo -e "${RED}错误: mtg-go 二进制文件无法执行。可能是架构错误。${PLAIN}"
    fi
...
create_service_mtg() {
    ...
    CMD_ARGS="simple-run -n 1.1.1.1 -t 30s -a 1mb -i only-ipv4 0.0.0.0:$PORT $FULL_SECRET"
...
menu() {
    ...
    echo -e "=================================="
    echo -e "${GREEN}1.${PLAIN} 安装/配置 MTProxy (Go 版)"
    echo -e "${GREEN}2.${PLAIN} 安装/配置 MTProxy (Python 版)"
    echo -e "----------------------------------"
