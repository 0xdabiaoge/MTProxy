...
install_telemt() {
    ...
    read -p "请输入【上行】速度限制 (MB/s, 例如 1.5, 回车则不限速): " SPEED_UP
    SPEED_UP=$(echo "$SPEED_UP" | tr -d '\r ' | xargs)
    SPEED_BLOCK=""
    if [ -n "$SPEED_UP" ]; then
        if ! awk -v value="$SPEED_UP" 'BEGIN { exit !(value ~ /^([0-9]+([.][0-9]+)?|[.][0-9]+)$/ && value > 0) }'; then
            echo -e "${RED}上行速度必须是大于 0 的数字。${PLAIN}"
            return 1
        fi
        ...
        if ! awk -v value="$SPEED_DOWN" 'BEGIN { exit !(value ~ /^([0-9]+([.][0-9]+)?|[.][0-9]+)$/ && value > 0) }'; then
            echo -e "${RED}下行速度必须是大于 0 的数字。${PLAIN}"
            return 1
        fi
        SPEED_BLOCK="[access.user_speed_limits]
$TELEMT_USER = \"$SPEED_UP $SPEED_DOWN\""
    fi
...
add_telemt_user() {
    ...
    if [ -n "$SPEED_UP" ]; then
        if ! awk -v value="$SPEED_UP" 'BEGIN { exit !(value ~ /^([0-9]+([.][0-9]+)?|[.][0-9]+)$/ && value > 0) }'; then
            echo -e "${RED}上行速度必须是大于 0 的数字，已取消添加。${PLAIN}"
            return
        fi
        read -p "请输入【下行】速度限制 (MB/s, 例如 5.0, 回车默认与上行相同): " SPEED_DOWN
        SPEED_DOWN=$(echo "$SPEED_DOWN" | tr -d '\r ' | xargs)
        [ -z "$SPEED_DOWN" ] && SPEED_DOWN=$SPEED_UP
        if ! awk -v value="$SPEED_DOWN" 'BEGIN { exit !(value ~ /^([0-9]+([.][0-9]+)?|[.][0-9]+)$/ && value > 0) }'; then
            echo -e "${RED}下行速度必须是大于 0 的数字，已取消添加。${PLAIN}"
            return
        fi
...
reset_telemt_user_quota() {
    ...
    if [ "$POL_OPT" -eq 1 ]; then
        ...
        if [ -n "$SPEED_UP" ]; then
            if ! awk -v value="$SPEED_UP" 'BEGIN { exit !(value ~ /^([0-9]+([.][0-9]+)?|[.][0-9]+)$/ && value > 0) }'; then
                echo -e "${RED}上行速度必须是大于 0 的数字，原限速保持不变。${PLAIN}"
                return
            fi
            ...
            [ -z "$SPEED_DOWN" ] && SPEED_DOWN=$SPEED_UP
            if ! awk -v value="$SPEED_DOWN" 'BEGIN { exit !(value ~ /^([0-9]+([.][0-9]+)?|[.][0-9]+)$/ && value > 0) }'; then
                echo -e "${RED}下行速度必须是大于 0 的数字，原限速保持不变。${PLAIN}"
                return
            fi
            NEW_SPEED="$SPEED_UP $SPEED_DOWN"
        else
