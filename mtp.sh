...
EOF
    ...
    B64_SECRET=$(printf '\xee%b%b' "$RAW_SECRET_BYTES" "$DOMAIN" | base64 -w0 | tr '+/' '-_' | tr -d '=')
...
show_detail_info() {
    ...
    if [ -f "$CONFIG_DIR/telemt.conf" ]; then
        ...
        RAW_SECRET_BYTES=$(echo -n "$SECRET" | sed 's/../\\x&/g')
        FULL_EE_SECRET=$(printf '\xee%b%b' "$RAW_SECRET_BYTES" "$DOMAIN" | base64 -w0 | tr '+/' '-_' | tr -d '=')
        show_info_telemt "$PORT" "$FULL_EE_SECRET" "$DOMAIN" "$IP_MODE"
    else
...
show_info_mtg() {
    ...
    FULL_SECRET=$(printf '\xee%b%b' "$RAW_SECRET_BYTES" "$3" | base64 -w0 | tr '+/' '-_' | tr -d '=')
    echo -e "=============================="
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        if [[ $in_users -eq 1 && -n "$line" && ! "$line" =~ ^# ]]; then
            ...
            if [ -n "$uName" ] && [ -n "$uSec" ]; then
                 ...
                 local B64_SECRET=$(echo -n "$full_secret" | xxd -r -p | base64 | tr '+/' '-_' | tr -d '=')
