...
check_all_status() {
    ...
    for SERVICE in mtg mtp-python mtp-rust; do
        local NAME=""
        case $SERVICE in
            mtg) NAME="Go     版 (mtg)" ;;
            mtp-python) NAME="Python 版" ;;
...
view_logs() {
    ...
    echo -e "${GREEN}2.${PLAIN} Python 版日志 (mtp-python)"
    echo -e "${GREEN}3.${PLAIN} Rust 版日志 (mtp-rust)"
    ...
    case $log_choice in
        1)
            ...
            ;;
        2)
            echo -e "${BLUE}=== Python 版日志 (最近 50 行) ===${PLAIN}"
            if [[ "$INIT_SYSTEM" == "systemd" ]]; then
                journalctl -u mtp-python -n 50 --no-pager
            else
                tail -n 50 /var/log/mtp-python.log 2>/dev/null || echo "日志文件不存在"
            fi
            ;;
        3)
        ...
        4)
            ...
            if [[ "$INIT_SYSTEM" == "systemd" ]]; then
                journalctl -u mtg -u mtp-python -u mtp-rust -f
            else
                tail -f /var/log/mtg.log /var/log/mtp-python.log /var/log/mtp-rust.log 2>/dev/null
            fi
            ;;
        0)
            return
...
control_service() {
    ACTION=$1
    shift
    TARGETS="mtg mtp-python mtp-rust"
    # 如果指定了具体服务名，就只操作那一个
...
delete_all() {
    ...
    if [[ "$INIT_SYSTEM" == "systemd" ]]; then
        systemctl disable mtg mtp-python mtp-rust 2>/dev/null
        rm -f /etc/systemd/system/mtg.service /etc/systemd/system/mtp-python.service /etc/systemd/system/mtp-rust.service
        systemctl daemon-reload
    else
        rc-update del mtg default 2>/dev/null
        rc-update del mtp-python default 2>/dev/null
        rc-update del mtp-rust default 2>/dev/null
        rm -f /etc/init.d/mtg /etc/init.d/mtp-python /etc/init.d/mtp-rust
    fi
...
menu() {
    ...
    echo -e "  Go 版: $(get_service_status_str mtg)  Python 版: $(get_service_status_str mtp-python)  Rust 版: $(get_service_status_str mtp-rust)"
    echo -e ""
    echo -e "  ${YELLOW}【安 装】${PLAIN}"
    echo -e "    ${GREEN}[1]${PLAIN} 安装 Go 版          ${GREEN}[2]${PLAIN} 安装 Python 版"
    echo -e "    ${GREEN}[3]${PLAIN} 安装 Rust 版"
    echo -e ""
    ...
    case $choice in
        ...
        3) install_base_deps; install_mtp_rust; back_to_menu ;;
        4) show_detail_info ;;
        5) modify_config ;;
        6) delete_config ;;
